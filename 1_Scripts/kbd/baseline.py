...
def predict(lang, mode = "dev", train_file = f"../../dataset/kbd.train.tsv"):

    

    allprules, allsrules = {}, {}
    # f"../../dataset/{lang}.train_2.tsv"
    ...
    if(mode == "dev"):
        ...
        evallines = [line.strip() for line in open(f"../../dataset/{lang}.test.tsv", "r") if line != '\n']
    
    outfile = open(f"../../2_Final_Submission/{lang}.txt", "w")
    ...
    if mode == "dev":
        # print(gold[:10])
        metric = evaluate(pred, gold)
        print(f"TRAINING FILE: {train_file} METRIC: {metric}")
        print()
...
def main():
    ...
    _, metric = predict(lang=lang, mode = mode, train_file = train_file)
